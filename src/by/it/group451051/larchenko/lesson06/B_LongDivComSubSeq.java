...
public class B_LongDivComSubSeq {
    ...
    int getDivSeqSize(InputStream stream) throws FileNotFoundException {
        ...
        // Инициализация: каждая позиция сама по себе является подпоследовательностью длины 1
        for (int i = 0; i < n; i++) {
            dp[i] = 1;
        }
        ...
        for (int i = 1; i < n; i++) {
            for (int j = 0; j < i; j++) {
                // Если текущий элемент делится на предыдущий без остатка,
                // то можно расширить подпоследовательность
